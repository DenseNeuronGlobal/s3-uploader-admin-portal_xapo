    ...
    <TableContainer component={Paper}>
      <Table>
        <TableHead className={classes.tableHead}>
          <TableRow>
            {showCheckBoxSelection && <TableCell align="left" className={classes.checkboxTableHeadCell} />}
            {columns.map((column: IColumn, key: number) => (
              <TableCell key={key} align="left" className={classes.tableHeadCell}>
                {column.title}
              </TableCell>
            ))}
          </TableRow>
            ...
            tableData.map((row: any, key: number) => (
              <TableRow key={key} hover={!!onRowClick} onClick={() => onRowClick && onRowClick(row)}>
                ...
                {columns.map((column: IColumn, columnKey: number) => (
                  <TableCell key={columnKey} align="left" className={classes.tableCell}>
